class TypingBox extends Component {
    constructor(element) {
        ...
        this.arabicSpecialChar = "أارزوؤدذ";
        languageChange.add(TypingBox.name, () => {
            this.reset();
        });
    }
    getChar(value, previous, next) {
        let before = "";
        let after = "";
    ...
    setUpDefaultMode(div, previousWords, isFirstRound) {
        ...
        window.onkeypress = (e) => {
            e.preventDefault();
            const typingInfo = Component.Components["TypingInfo"][0];
                    ...
                    if (nextLines.length < 4) {
                        ...
                        for (let i = 0; i < nextLines.length; i++) {
                            for (let y = 0; y < nextLines[i].length; y++) {
                                const chars = Array.from(nextLines[i][y].querySelectorAll('.char'));
    ...
    onRendered() {
        setTimeout(() => {
            let line = 0;
            let currentBottom;
            const children = Array.from(this.shadowDiv.children);
    ...
    onFinish() {
        ...
        Component.GetComponents(TypingInfo)[0].hide();
        const analysisBox = new TypingAnalysisBox(document.querySelector("#content"));
